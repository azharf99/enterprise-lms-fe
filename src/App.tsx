...
const App: React.FC = () => {
  ...
  return (
    <BrowserRouter>
    <Suspense fallback={<div className="h-screen w-screen flex items-center justify-center bg-gray-50 text-blue-600 font-bold tracking-tight">Enterprise Learning...</div>}>
      <Routes>
        {/* Rute Publik */}
        ...
        <Route path="*" element={!role ? <Navigate to="/login" /> : (role === 'Siswa' ? <Navigate to="/student-dashboard" /> : <Navigate to="/dashboard" />)} />
      </Routes>
      </Suspense>
    </BrowserRouter>
  );
