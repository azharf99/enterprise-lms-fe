...
export const ExamQuestionManagement: React.FC = () => {
  ...
  const [aiPrompt, setAiPrompt] = useState({ topic: '', type: 'MultipleChoice', count: 5 });
  ...
  const openManualModal = (mode: 'create' | 'edit', q?: ExamQuestion) => {
    ...
    else setFormData({ type: 'MultipleChoice', text: '', options: ['', '', '', ''], correct_answer: '', points: 10, explanation: '' });
    setIsModalOpen(true);
  };
            ...
            <form onSubmit={handleAIGenerate} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={aiPrompt.type} onChange={e => setAiPrompt({...aiPrompt, type: e.target.value})} className="w-full border-2 border-gray-200 rounded-lg p-3 focus:border-purple-500 outline-none">
                ...
                <div>
                  ...
                  <input type="number" min="1" max="20" required value={aiPrompt.count} onChange={e => setAiPrompt({...aiPrompt, count: Number(e.target.value)})} className="w-full border-2 border-gray-200 rounded-lg p-3 focus:border-purple-500 outline-none" />
                </div>
              </div>
